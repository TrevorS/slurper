...
/// htdemucs (drums, bass, other, vocals; Meta, MIT) on Core AI.
///
...
/// `scripts/convert_htdemucs.py` produced the model.
///
/// Segments run one at a time: two runs at once, even on separate functions of one model, corrupt the
/// outputs (drums at 12 dB SDR against PyTorch), and would save 17% on a 135 s song.
final class Demucs: Sendable {
    ...
    private let scalarType: NDArray.ScalarType
    ...
    init(model url: URL) async throws {
        ...
        guard let function = try model.loadFunction(named: "main"),
              case .ndArray(let input) = function.descriptor.inputDescriptor(of: "mix")
        else { throw SplitError.model("the Demucs model has no main(mix, spec) function") }
    ...
    func stems(ofSegment mix: [[Float]]) async throws -> [[[Float]]] {
        ...
        var outputs = try await function.run(inputs: [
            "mix": array(mix[0] + mix[1], shape: [1, 2, segment]),
            "spec": array(spec, shape: [1, 4, Self.bins, Self.frames]),
        ])
        guard let time = outputs.remove("time")?.ndArray, let freq = outputs.remove("freq")?.ndArray else {
            throw SplitError.model("the Demucs model returned no time or freq output")
        }
        let timeValues = try values(time, count: 8 * segment)
        let freqValues = try values(freq, count: 16 * planes)
    ...
    private func array(_ values: [Float], shape: [Int]) -> NDArray {
        var input = NDArray(shape: shape, scalarType: scalarType)
        switch scalarType {
        case .float16: fill(&input, with: values.map(Float16.init))
        default: fill(&input, with: values)
        }
        return input
    }

    private func values(_ output: NDArray, count: Int) throws -> [Float] {
        switch output.scalarType {
        case .float16: read(output, Float16.self, count: count).map(Float.init)
        case .float32: read(output, Float.self, count: count)
        default: throw SplitError.model("unsupported Demucs output type \(output.scalarType)")
        }
    }
}
