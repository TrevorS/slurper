...
test: ## Unit tests with a coverage report
	...
	xcrun xccov view --report --json build/Tests.xcresult | python3 scripts/coverage.py

install: build ## Build and install to ~/.local/bin (override with PREFIX=...)
	mkdir -p $(PREFIX)/bin
	cp $(PRODUCTS)/slurper $(PREFIX)/bin/slurper

model: ## Rebuild the htdemucs Core AI model from PyTorch into ~/Library/Application Support/Slurper/Models (needs uv)
